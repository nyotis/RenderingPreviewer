...
driver_open {
   ...
   if ( data->media_name == NULL ) {
         ...
         // Generate a unique media name: it cannot contain spaces. Everything after the last 
...
/*         aov_cmds = oss.str() + aov_cmds;*/
   ...
   data->nchannels = 4;
      // Convert to RGBA, since RV does not allow layers with differing types/channels

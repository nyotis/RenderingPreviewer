...
class: MayaRenderMode : rvtypes.MinorMode {
   ...
   method: singleRenderRegion (void; Event e) {
       ...
       let percent_image_float_min = commands.eventToImageSpace(sName, rvtypes.Point(_startx, _starty), false);
          //print(percent_image_float_min);
          // [percent_image_float_min.x, percent_image_float_min.y]

       let percent_image_float_max = commands.eventToImageSpace(sName, rvtypes.Point(_endx, _endy), false);
       ...
       float coord1 = percent_image_float_min.x / image_aspect * w;
       float coord2 = percent_image_float_min.y * h;
       
       clamp(coord1);
       clamp(coord2);

       float coord3 = percent_image_float_max.x / image_aspect * w;
       float coord4 = percent_image_float_max.y * h;
       ...
       // this doesn't work 
       /*if (coord1 > coord3) {
          sendMayaCommand( "bb_renderRegion %f %f %f %f;" % ( coord3, coord2, coord1, coord4 ) );
       }
       else {
          sendMayaCommand( "bb_renderRegion %f %f %f %f;" % ( coord1, coord2, coord3, coord4 ) );  
       }*/


       if (coord2 > coord4) {
          sendMayaCommand( "bb_renderRegion %f %f %f %f;" % ( coord1, coord4, coord3, coord2 ) );
       }
       else {
          sendMayaCommand( "bb_renderRegion %f %f %f %f;" % ( coord1, coord2, coord3, coord4 ) );  
       }
